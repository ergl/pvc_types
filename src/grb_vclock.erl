...
-spec set_max_time(T, non_neg_integer(), vc(T)) -> vc(T).
...
-spec lt_at_keys([T], vc(T), vc(T)) -> boolean().
lt_at_keys(Keys, Left, Right) ->
    leq_at_keys(Keys, Left, Right) andalso begin
        lists:any(fun(Key) ->
            get_time(Key, Left) < get_time(Key, Right)
        end, Keys)
    end.

-spec to_list(vc(T)) -> [{T, non_neg_integer()}].
to_list(VC) -> maps:to_list(VC).

-ifdef(TEST).

-spec from_list({T, non_neg_integer()}) -> vc(T).
from_list(L) ->
    maps:from_list(L).
...
grb_vclock_lt_at_keys_test() ->
    ?assertEqual(true, grb_vclock:lt_at_keys([a,b,c],
        #{c => 0},
        #{c => 1}
    )),

    ?assertEqual(false, grb_vclock:lt_at_keys([a,b,c], #{}, #{})).
